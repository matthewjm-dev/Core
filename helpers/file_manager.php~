...
if ($raw_file["size"] <= ipsCore_file_manager::$max_upload_size) {
	...
	public static function get_url_details( $url )
	{
		$details = [
			'dirname' => false,
			'basename' => false,
    	...
    	$details = array_merge( $details, pathinfo( $url ), getimagesize( $url ) );

    	if (!$details['extension'] && $details['mime']) {
    		$details['extension'] = str_replace('image/', '', $details['mime']);
		}

		if (!$details['mime'] && $details['extension']) {
			$details['mime'] = 'image/' . $details['extension'];
		}

		$details['basename'] = ltrim($details['basename'], '?');
		$details['filename'] = ltrim($details['filename'], '?');
	...
	{
		$image_details = self::get_url_details($url);
		...
		if ( $image_details ) {
			if ( in_array( $image_details[ 'extension' ], ipsCore_file_manager::$allowed_types_images ) ) {
				...
				if ( $dirCheck === true ) {
					...
					$image_path = $path . $image_details[ 'filename' ] . "." . $image_details[ 'extension' ];
					...
					if ( file_put_contents( $image_path, file_get_contents( $url ) ) ) {
						return [
							'basename'  => $image_details[ 'filename' ] . '.' . $image_details[ 'extension' ],
